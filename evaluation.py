...
def get_topic_qualities(topic_word_list, palmetto_dir, **kwargs):
    ...
    os.system(f'java -jar {palmetto_dir}/palmetto-0.1.5-exec.jar {palmetto_dir}/wikipedia_bd umass {filename} > {filename[:-4]}_umass.txt')
    with open(f"{filename[:-4]}_umass.txt", 'r') as f:
        tmpstr = '\n'.join(f.readlines())
    umass = read_palmetto_result(tmpstr)
    os.system(f'java -jar {palmetto_dir}/palmetto-0.1.5-exec.jar {palmetto_dir}/wikipedia_bd npmi {filename} > {filename[:-4]}_npmi.txt')
    with open(f"{filename[:-4]}_npmi.txt", 'r') as f:
        tmpstr = '\n'.join(f.readlines())
    npmi = read_palmetto_result(tmpstr)
    os.system(f'java -jar {palmetto_dir}/palmetto-0.1.5-exec.jar {palmetto_dir}/wikipedia_bd c_p {filename} > {filename[:-4]}_cp.txt')
    with open(f"{filename[:-4]}_cp.txt", 'r') as f:
        tmpstr = '\n'.join(f.readlines())
    cp = read_palmetto_result(tmpstr)
    os.system(f'java -jar {palmetto_dir}/palmetto-0.1.5-exec.jar {palmetto_dir}/wikipedia_bd uci {filename} > {filename[:-4]}_uci.txt')
...
def read_palmetto_result(result_text):
    ...
    for line in result_lines:
        if line.strip() == '':  # 빈 줄 처리
            continue
        parts = line.split('\t')
        if len(parts) < 2:  # 탭으로 분리된 부분이 2개 미만일 경우 처리
            print(f"Unexpected line format: {line}")
            continue
        try:
            val = float(parts[1])  # float 변환 시도
            val_l.append(val)
        except ValueError:  # float 변환 실패 시 처리
            print(f"Cannot convert value to float: {parts[1]}")
            continue
    if len(val_l) == 0:  # val_l이 비어있을 경우 처리
        print("No values found.")
        return 0
    average = sum(val_l) / len(val_l)
    print(val_l)
    print(average)
    return average
